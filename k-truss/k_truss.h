...
class k_truss
{
	...
	string path;                    // 文件路径holds the path for the file
	string path_name;               // 文件名only the name without the .txt

	int n, m;                       // vetexes reading from the file
	vector<int> mapto;              // 用于排序的容器 container that is used for sorting
	vector<int> deg, bin;           // 存储顶点的度数holds the degrees for the vertexes 
	vector<Edge> binEdge;           // 分类容器container for sorting
	vector<vector<int>> A;          // 三角形计数容器container for couting triangles 
	vector<map<int, int>> Adj, pos;  // 调整矩阵进行初始化Adjusted matrix for initialization

	int cntClass[1000];             // 保持类holds the classes for the 
